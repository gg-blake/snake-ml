from .neural_network_dl import DoubleLayerNeuralNetwork
...
class DLPopulation:
    def __init__(self, size: int, input_nodes, first_hidden_nodes, second_hidden_nodes, output_nodes, mutation_rate: float, population=None):
        # Instance Variables #
    ...
    def breed(self, parent_a: Snake, parent_b: Snake) -> Snake:
        ...
        child_nn = DoubleLayerNeuralNetwork(self.input_nodes, self.first_hidden_nodes, self.second_hidden_nodes, self.output_nodes, parent_a.nn.id, parent_b.nn.id)
        child = Snake(nn=child_nn)

        # Child inherits average weights and bias of the parents
        child.nn.weights_ih1 = np.add(parent_a.nn.weights_ih1, parent_b.nn.weights_ih1) / 2
        child.nn.weights_h1h2 = np.add(parent_a.nn.weights_h1h2, parent_b.nn.weights_h1h2) / 2
        child.nn.weights_h2o = np.add(parent_a.nn.weights_h2o, parent_b.nn.weights_h2o) / 2
        child.nn.bias_h1 = np.add(parent_a.nn.bias_h1, parent_b.nn.bias_h1) / 2
        child.nn.bias_h2 = np.add(parent_a.nn.bias_h2, parent_b.nn.bias_h2) / 2
    ...
    def load_from_json(self, filename: str):
        if os.path.exists(f"./snake-ml/dl_data/{filename}"):
            with open(f"./snake-ml/dl_data/{filename}", "r") as f:
                data = json.load(f)

                self.generation = data["generation"]
                for i, v in enumerate(data["population"]):
                    self.population[i].nn.input_nodes = v['input_nodes']
                    self.population[i].nn.first_hidden_nodes = v['first_hidden_nodes']
                    self.population[i].nn.second_hidden_nodes = v['second_hidden_nodes']
                    self.population[i].nn.output_nodes = v['output_nodes']
                    self.population[i].nn.weights_ih1 = np.array(v['weights_ih1'])
                    self.population[i].nn.weights_h1h2 = np.array(v['weights_h1h2'])
                    self.population[i].nn.weights_h2o = np.array(v['weights_h2o'])
                    self.population[i].nn.bias_h1 = np.array(v['bias_h1'])
                    self.population[i].nn.bias_h2 = np.array(v['bias_h2'])
    ...
    def save_to_json(self, filename: str):
        ...
        with open(f"./snake-ml/dl_data/{filename}", "w") as f:
