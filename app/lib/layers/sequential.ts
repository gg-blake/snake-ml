...
export default class FeedForward extends GameLayer {
    ...
    build() {
        ...
        this.wHiddenOutput = this.addWeight('wHiddenOutput', [this.B, (((this.C - 1) * this.C)) + 1, this.units], this.dtype, tf.initializers.randomUniform({ minval: -1, maxval: 1 }))
        this.bHiddenOutput = this.addWeight('bHiddenOutput', [this.B, (((this.C - 1) * this.C)) + 1, 1], this.dtype, tf.initializers.randomUniform({ minval: -1, maxval: 1 }))
    }

    call(inputs: [tf.Tensor3D]): [tf.Tensor2D, tf.Tensor1D] {
        ...
        return tf.tidy(() => {
            ...
            const control = logits.squeeze([2]).slice([0, ((this.C - 1) * this.C)], [this.B, 1]).squeeze([1]) as tf.Tensor1D;
            const softmax = tf.softmax<tf.Tensor2D>(logits.squeeze([2]).slice([0, 0], [this.B, ((this.C - 1) * this.C)]) as tf.Tensor2D, 1); // (B, (((C-1)*C)/2))
            return [tf.keep(softmax), tf.keep(control)]
        });
    }

    computeOutputShape(inputShape: tf.Shape | tf.Shape[]): tf.Shape | tf.Shape[] {
        //return inputShape;
        return [[this.B, (((this.C - 1) * this.C) / 2)], [this.B, 1]];
    }
