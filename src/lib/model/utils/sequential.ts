...
export default class FeedForward extends tf.layers.Layer {
    ...
    constructor(config: LayerArgs & Config, seed?: number) {
        super(config);
        this.config = config;
        this.seed = seed;
    }
    ...
    call(inputs: [tf.Tensor3D]): tf.Tensor2D {
        return tf.tidy(() => {
            ...
            const hidden = tf.matMul(ih, inputs[0], false, false).clipByValue(-1, 1);
            const logits = tf.matMul(ho, hidden, false, false).clipByValue(-1, 1).squeeze([2]);
