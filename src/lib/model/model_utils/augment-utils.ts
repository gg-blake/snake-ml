...
export function dotProduct<T extends tf.Tensor>(
    A: T,
    B: T,
    axis?: number,
    keepDims?: boolean,
...
export const project = (U: tf.Tensor2D, V: tf.Tensor2D): tf.Tensor2D =>
    tf.tidy(() => {
        ...
        if (U.shape[0] != V.shape[0] || U.shape[1] != V.shape[1]) {
            throw new Error(
                `U [Shape: ${U.shape}] and V [Shape: ${V.shape}] must have matching shapes`,
            );
        }
...
export const cosineSimilarity = (
    ...
    tf.tidy(() => {
        ...
        const div1 = dot1.div(
            tf.where(dot2.greater(epsilon), dot2, epsilon),
        ) as tf.Tensor1D;
        return div1;
    });
...
export const projectToPlane = (
    ...
    tf.tidy(() => {
        ...
        if (
            ...
            throw new Error(
                `A [Shape: ${A.shape}], B [Shape: ${B.shape}], C [Shape: ${C.shape}] must have matching shapes`,
            );
        }
...
export const unsignedPlaneAngle = (
    A: tf.Tensor2D,
    B: tf.Tensor2D,
    C: tf.Tensor2D,
): tf.Tensor1D =>
...
export const signedPlaneAngle = (
    ...
    tf.tidy(() => {
        ...
        const unsignedThetaPlaneBA = tf.sub(
            Math.PI / 2,
            unsignedPlaneAngle(B, A, C),
        );
        const sign = unsignedThetaPlaneBA.div(unsignedThetaPlaneBA.abs());
        const theta = unsignedThetaPlaneAB.mul(
            sign.where(sign.isNaN().logicalNot(), tf.zeros(sign.shape)),
        ) as tf.Tensor1D;
        return theta;
    });
...
export function generatePlaneIndices(n: number): tf.Tensor2D {
    return tf.tidy(() => {
        return tf.tensor2d(
            Array.from(Array(n - 1).keys(), (v: number) => [0, v + 1]),
            ...[,],
            "int32",
        );
    });
